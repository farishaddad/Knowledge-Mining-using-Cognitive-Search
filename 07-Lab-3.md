...
Now let's use the [official documentation](https://docs.microsoft.com/en-us/azure/search/cognitive-search-custom-skill-interface) to learn the sintax we need to add the custom skill to our enrichment pipeline. 
...
####Step 7.1 - Challenge!!
As you can see, again we are not giving you the body request. One more time you need to use LAB 1 body definition json files and add instructions to the end of skillset, index and indexer definitions. We can't use lab 2 definition because on the maximum number os skills within a skillset of a basic account, 5 is the limit. So, let's use LAB 1 json files again.
...
##Finished Solution
If you could not make it, [here](Finished-Solutoin-Lab-3.md) is the challenge solution. You just need to folow the steps.
